...
function BotCard({ bot, onSelect, index }: { bot: Bot; onSelect: () => void; index: number }) {
  return (
    <button
      onClick={onSelect}
      className="group relative bg-white rounded-xl sm:rounded-2xl border border-[rgba(30,58,47,0.08)] p-3 sm:p-5 text-left transition-all duration-300 hover:shadow-xl hover:shadow-[#4A7C59,0.08)] hover:border-[#4A7C5940] active:scale-[0.99] sm:hover:-translate-y-1 card-glow animate-fadeInUp overflow-hidden"
      ...
      <div className="relative flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 sm:mb-1.5">
            <h3 className="font-semibold text-[#1E3A2F] group-hover:text-[#4A7C59] transition-colors truncate text-sm sm:text-base">
              {bot.name}
            </h3>
            {bot.isNew && (
          ...
          <p className="text-xs sm:text-sm text-[#6B6B6B] line-clamp-2 leading-relaxed mb-2 sm:mb-3">
            {bot.description}
          </p>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 text-[#8B8B8B] group-hover:text-[#4A7C59] transition-colors">
  ...
  return (
    <button
      ...
      className={`group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
        isSelected
          ? 'bg-gradient-to-r from-[#4A7C59] to-[#1E3A2F] text-white shadow-lg shadow-[#4A7C5925]'
          : 'bg-white text-[#1E3A2F] hover:bg-[#4A7C59,0.05)] active:bg-[#4A7C59,0.1)] hover:text-[#4A7C59] border border-[rgba(30,58,47,0.08)]'
      }`}
    >
      {category ? (
        <>
        ...
        <>
          <FiStar className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isSelected ? '' : 'text-[#8B8B8B] group-hover:text-[#4A7C59]'}`} />
          <span>Todos</span>
        </>
      )}
      <span
        className={`px-1 sm:px-1.5 py-0.5 rounded text-[10px] sm:text-xs ${
          isSelected ? 'bg-white/20' : 'bg-[rgb(245,245,250)] text-[#6B6B6B]'
        }`}
      >
        {count}
      </span>
...
export default function BotsGrid() {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-gradient-to-b from-[#F5F5F0] to-white overflow-hidden">
      {/* Header */}
      <header className="px-4 sm:px-6 py-3 sm:py-5 bg-white/80 backdrop-blur-lg border-b border-[rgba(30,58,47,0.08)]">
        <div className="flex items-center gap-3">
          ...
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-[#4A7C59] to-[#1E3A2F] flex items-center justify-center shadow-lg shadow-[#4A7C5920] flex-shrink-0">
            <FiZap className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
          </div>
          <div className="min-w-0">
            <h1 className="text-base sm:text-xl font-bold text-[#1E3A2F]">Ferramentas</h1>
      ...
      <div className="px-4 sm:px-6 py-3 sm:py-4 bg-white/50 backdrop-blur-sm border-b border-[rgba(30,58,47,0.05)] sticky top-0 z-10">
      ...
      {selectedCategory === null && (
        <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gradient-to-r from-[#4A7C5908] to-transparent">
          <div className="flex items-center gap-2 text-xs sm:text-sm font-medium text-[#4A7C59]">
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 pb-6 pt-2">
        ...
        {filteredBots.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 sm:py-16 text-center">
            ...
            <h3 className="text-base sm:text-lg font-semibold text-[#1E3A2F] mb-2">
